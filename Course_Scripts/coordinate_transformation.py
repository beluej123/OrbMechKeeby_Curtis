...
vf_vec = np.array(
    [-np.sqrt(mu / p) * np.sin(oe[5]), np.sqrt(mu / p) * (oe[1] + np.cos(oe[5])), 0]
)
...
print(
    f"Check r_inertial (not required): r_inertial = {np.linalg.norm(r_vec_inertial)} km. It must have the same value of r."
)
